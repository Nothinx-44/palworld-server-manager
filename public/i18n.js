...
(function () {
  ...
  const T = {
    ...
    'Configurer / réinstaller': 'Configure / reinstall',
    ...
    'La détection lit le jeton dans': 'Detection reads the token from',
    "(il faut avoir activé l'API dans": '(you must have enabled the API in',
    'et créé un jeton, voir la doc PalDefender). Le dashboard ne modifie jamais les fichiers du plugin.': 'and created a token, see the PalDefender docs). The dashboard never modifies plugin files.',
    ...
    'Échec de la détection': 'Detection failed',
    // Comptes
  ...
  const PATTERNS = [
    ...
    [/^Jeton détecté \((.+)\) et enregistré$/, 'Token detected ($1) and saved'],
    [/^Jeton détecté \((.+)\) — pense à mettre Enabled: true dans RESTConfig\.json puis à redémarrer le serveur$/, 'Token detected ($1) — remember to set Enabled: true in RESTConfig.json then restart the server'],
    [/^Échec : (.+)$/, 'Failed: $1'],
    ...
    // Libellés du journal d'activité ("<pseudo> a fait X")
