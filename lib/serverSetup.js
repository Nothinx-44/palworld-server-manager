...
async function setupNssmService({ serverDir, serverName, port, maxPlayers, queryPort, extraArgs }, onLog) {
  // Une config sauvegardée par une version antérieure à l'ajout du champ queryPort n'en a pas :
  // sans ce repli, le service serait créé avec "-queryport=undefined" dans ses paramètres.
  ...
  const baseParams = `-ServerName="${serverName}" -port=${port} -queryport=${queryPort} -players=${maxPlayers} -useperfthreads -NoAsyncLoadingThread -UseMultithreadForDS EpicApp=PalServer`;
  const appParams = extraArgs ? `${baseParams} ${extraArgs}` : baseParams;
...
function normalizeConfig(body = {}) {
  ...
  return {
    ...
    // sans re-télécharger ni écraser ses mots de passe. Les flags *Provided distinguent "laissé
    // vide pour conserver l'existant" (import) de "laissé vide = valeur par défaut" (install neuve).
    existingServer: !!body.existingServer,
    serverPasswordProvided: body.serverPassword !== undefined && body.serverPassword !== '',
    serverNameProvided: !!body.serverName,
    ...
    // Arguments de lancement supplémentaires passés tels quels à PalServer.exe via NSSM
    // (ex: -publiclobby, demandé dans l'issue #5). Champ avancé, admin uniquement — collé en fin
    // de ligne de commande. Les sauts de ligne/retours chariot sont retirés (une AppParameters
    // NSSM est mono-ligne).
    extraArgs: String(body.extraArgs || '').replace(/[\r\n]+/g, ' ').trim()
  };
}
...
async function getStatus() {
  ...
  return {
    ...
    current: {
      ...
      // Crucial pour un serveur importé : sans ce flag restauré dans le formulaire, une
      // réinstallation traiterait installDir comme un dossier parent et retéléchargerait le
      // serveur dans un sous-dossier "Server" au lieu de réutiliser l'existant.
      existingServer: !!saved.existingServer,
...
async function runInstall(config, onLog) {
  ...
  if (!isDefaultSettingsReady(serverDir)) {
    // Le 1er lancement (génération de DefaultPalWorldSettings.ini) échoue par intermittence sur
    // certaines machines (antivirus scannant PalServer.exe, disque lent) — un seul essai raté
    // interrompait toute l'install AVANT applyIniSettings, laissant un serveur à moitié configuré
    // (ports/mots de passe jamais écrits dans le .ini). On réessaie donc plusieurs fois avant
    // d'abandonner, comme pour SteamCMD. (Signalé dans l'issue GitHub #5.)
    const MAX_FIRSTLAUNCH_ATTEMPTS = 3;
    for (let attempt = 1; ; attempt++) {
      onLog(attempt === 1
        ? 'Premier lancement pour générer la config...'
        : `Config toujours pas générée — nouvelle tentative de premier lancement (${attempt}/${MAX_FIRSTLAUNCH_ATTEMPTS})...`);
      try {
        await firstLaunch(serverDir, onLog);
        break;
      } catch (err) {
        if (attempt >= MAX_FIRSTLAUNCH_ATTEMPTS) throw err;
        onLog(`(Premier lancement non abouti : ${err.message} — nouvelle tentative.)`);
      }
    }
  } else {
  ...
  if (config.existingServer && fs.existsSync(settingsFile)) {
    ...
    // Même principe pour le nom du serveur : champ laissé vide = on conserve le nom déjà
    // configuré, plutôt que d'écraser silencieusement avec le défaut "Serveur Palworld".
    if (!config.serverNameProvided && findVal('ServerName')) {
      config.serverName = findVal('ServerName');
    }
  }
  ...
  await setupNssmService({ serverDir, serverName: config.serverName, port: config.port, queryPort: config.queryPort, maxPlayers: config.maxPlayers, extraArgs: config.extraArgs }, onLog);
  ...
  updateEnvFile({
    ...
    // Serveur importé sans SteamCMD : ne pas écrire un chemin inexistant, la vérification de
    // mise à jour Steam échouerait avec un ENOENT incompréhensible au lieu d'un message clair.
    ...(fs.existsSync(steamCmdExe) ? { STEAMCMD_PATH: steamCmdExe } : {}),
