...
// L'API REST officielle Palworld renvoie le champ IP du joueur sous la clé "iP" (P majuscule),
// pas "ip" — confirmé sur un vrai serveur (voir doc officielle qui montre "ip" en minuscule dans
// ses exemples, mais la réponse réelle utilise "iP"). Sans cette normalisation, l'IP est toujours
// undefined côté dashboard (colonne IP vide, historique sans IP) alors que la donnée est bien là.
function normalizePlayers(players) {
  return (players || []).map(p => ({ ...p, ip: p.ip || p.iP || null }));
}
...
module.exports = { getPalworldApi, runNssm, isServiceRunning, gracefulStop, killOrphanGameProcesses, getNssmPath, getServiceName, normalizePlayers };
