...
async function poll() {
    ...
    await updateJson(HISTORY_FILE, emptyHistory(), history => {
      ...
      players.forEach(p => {
        if (!onlineSince[p.userId]) {
          ...
          notifications.push(`🟢 **${p.name}** a rejoint le serveur`);
        } else {
      ...
      Object.keys(onlineSince).forEach(userId => {
        ...
        notifications.push(`🔴 **${name}** a quitté le serveur (${durationMin} min de jeu)`);
      });
  ...
  notifications.forEach(msg => discord.notify(msg));
}
